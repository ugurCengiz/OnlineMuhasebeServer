...
namespace OnlineMuhasebeServer.Persistance.Services.AppServices
{
    public sealed class MainRoleService : IMainRoleService
    {
        ...
        {
            if (companyId == null) return await _mainRoleQueryRepository.GetFirstByExpiression(x => x.Title == title,cancellationToken);
            return await _mainRoleQueryRepository.GetFirstByExpiression(x =>
                x.Title == title && x.CompanyId == companyId,cancellationToken,false);
        }
        ...
        {
            return _mainRoleQueryRepository.GetAll(false);
        }
